...
class VoiceListener:
    ...
    def _load_big_model(self):
        print()
        print("[Voice] Гружу точную модель в фоне...")
        ...
        try:
            ...
            warmup_recognizer = vosk.KaldiRecognizer(
                self.big_model, self.samplerate
            )
    ...
    def _run(self):
        try:
            print()
            print("[Voice] Гружу лёгкую модель...")
            ...
            print(f"[Voice] Sample rate: {samplerate}")

            try:
                self.recognizer = vosk.KaldiRecognizer(
                    self.small_model, samplerate
                )
            except Exception as error:
            ...
            print()
            print(f"[Voice] Готов. Триггер: «{self.trigger_word}»")
            print("[Voice] Голосовой listener запущен.")
    ...
    # ==================================================
    # TRIGGER MATCHING (нечёткое)
    # ==================================================

    def _find_trigger_index(self, words: list) -> int | None:
        if self.trigger_word in words:
            return words.index(self.trigger_word)

        close = difflib.get_close_matches(
            self.trigger_word,
            words,
            n=1,
            cutoff=self.TRIGGER_MATCH_CUTOFF,
        )

        if close:
            return words.index(close[0])
    ...
    def _handle_phrase(self, quick_text: str, audio_bytes: bytes):
        ...
        if not self._is_actionable(quick_text):
            return
    ...
    def _recognize_precisely(
        self, fallback_text: str, audio_bytes: bytes
    ) -> str:
        ...
        if self.stt_mode == "online":
            try:
                return recognize_with_groq(audio_bytes, self.samplerate)
            except Exception as error:
